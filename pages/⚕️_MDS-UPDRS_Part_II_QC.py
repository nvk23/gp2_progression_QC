...
from utils.app_setup import MDS_UPDRS_PT2
...
# Page set-up
mds_updrs_pt2 = MDS_UPDRS_PT2('MDS-UPDRS Part 2 QC', 'mds_updrs_pt2')
mds_updrs_pt2.config_page()
mds_updrs_pt2.config_MDS_UPDRS_PT2()
mds_updrs_pt2.google_connect()

# Uploader set-up
data_file = mds_updrs_pt2.config_data_upload(mds_updrs_pt2.TEMPLATE_LINK)
...
if data_file is not None and study_name is not None:
    ...
    missing_outcome, available_metrics = prep_merge(mds_updrs_pt2, uploaded_df)
    ...
    df = mds_updrs_scores(mds_updrs_pt2, available_metrics, df)
    ...
    if 'mds_updrs_pt2_counter' not in st.session_state:
        ...
        qc_col1.selectbox(
            "Choose an MDS UPDRS Part 1 metric", st.session_state['mds_updrs_pt2_variable'], label_visibility='collapsed')
        qc_col2.button("Continue", on_click=mds_updrs_pt2.call_on, args = ['mds_updrs_pt2_btn'])
        get_varname = None
    else:
        qc1.markdown('### MDS UPDRS Part II Quality Control')
        st.session_state['mds_updrs_pt2_counter'] += 1
        if len(st.session_state['mds_updrs_pt2_variable']) >= 1:
            mds_updrs_pt2_version = qc_col1.selectbox(
                "Choose an MDS UPDRS Part II metric",st.session_state['mds_updrs_pt2_variable'], on_change=mds_updrs_pt2.call_off, args = ['mds_updrs_pt2_btn'], label_visibility='collapsed')
            get_varname = mds_updrs_pt2.OUTCOMES_DICT[mds_updrs_pt2_version]
            qc_col2.button("Continue", on_click = mds_updrs_pt2.call_on, args = ['mds_updrs_pt2_btn'])
        else:
            st.markdown(
            ...
            final_df = reduce(lambda x, y: pd.merge(x, y,
                                                    on=['clinical_id',
                                                        'visit_month'],
                                                    how='outer'), st.session_state['mds_updrs_pt2_data_chunks'])

    if st.session_state['mds_updrs_pt2_btn']:

        # reorder columns for display
        df_subset = mds_updrs_pt2.reorder_cols(df)
        ...
        # may need to move this higher to the initial QC before mds_updrs_pt2-specific QC
        st.markdown('Select a stratifying variable to plot:')
        plot1, plot2, plot3 = st.columns(3)
        strata = plot1.selectbox("Select a stratifying variable to plot:", mds_updrs_pt2.STRAT_VALS.keys(
        ), index=0, label_visibility='collapsed', on_change=mds_updrs_pt2.call_off, args=['mds_updrs_pt2_plot_val'])
        selected_strata = mds_updrs_pt2.STRAT_VALS[strata]

        # Make sure selected stratifying variable is in the dataframe
        check_strata(df_final, selected_strata)

        plot2.button('Continue', key='continue_plot', on_click = mds_updrs_pt2.call_on, args = ['mds_updrs_pt2_plot_val'])
        plot_outcomes(mds_updrs_pt2, df_final, get_varname, mds_updrs_pt2_version, selected_strata)
        review_sample(mds_updrs_pt2, df_final)
        qc_submit(mds_updrs_pt2, df_final, df_subset, study_name)
