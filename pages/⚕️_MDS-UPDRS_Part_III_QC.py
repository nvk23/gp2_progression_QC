...
from utils.app_setup import MDS_UPDRS_PT3
...
# Page set-up
mds_updrs_pt3 = MDS_UPDRS_PT3('MDS-UPDRS Part 3 QC', 'mds_updrs_pt3')
mds_updrs_pt3.config_page()
mds_updrs_pt3.config_MDS_UPDRS_PT3()
mds_updrs_pt3.google_connect()

# Uploader set-up
data_file = mds_updrs_pt3.config_data_upload(mds_updrs_pt3.TEMPLATE_LINK)
...
if data_file is not None and study_name is not None:
    ...
    missing_outcome, available_metrics = prep_merge(mds_updrs_pt3, uploaded_df)
    ...
    df = mds_updrs_scores(mds_updrs_pt3, available_metrics, df)
    ...
    if 'mds_updrs_pt3_counter' not in st.session_state:
        ...
        if len(st.session_state['mds_updrs_pt3_variable']) >= 1:
            ...
            final_df = reduce(lambda x, y: pd.merge(x, y,
                                                    on=['clinical_id',
                                                        'visit_month'],
                                                    how='outer'), st.session_state['mds_updrs_pt3_data_chunks'])

    if st.session_state['mds_updrs_pt3_btn']:

        # reorder columns for display
        df_subset = mds_updrs_pt3.reorder_cols(df)
        ...
        # may need to move this higher to the initial QC before mds_updrs_pt3-specific QC
        st.markdown('Select a stratifying variable to plot:')
        plot1, plot2, plot3 = st.columns(3)
        strata = plot1.selectbox("Select a stratifying variable to plot:", mds_updrs_pt3.STRAT_VALS.keys(
        ), index=0, label_visibility='collapsed', on_change=mds_updrs_pt3.call_off, args=['mds_updrs_pt3_plot_val'])
        selected_strata = mds_updrs_pt3.STRAT_VALS[strata]

        # Make sure selected stratifying variable is in the dataframe
        check_strata(df_final, selected_strata)

        plot2.button('Continue', key='continue_plot', on_click = mds_updrs_pt3.call_on, args = ['mds_updrs_pt3_plot_val'])
        plot_outcomes(mds_updrs_pt3, df_final, get_varname, mds_updrs_pt3_version, selected_strata)
        review_sample(mds_updrs_pt3, df_final)
        qc_submit(mds_updrs_pt3, df_final, df_subset, study_name)
